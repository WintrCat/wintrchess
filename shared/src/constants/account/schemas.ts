...
export const email = z.string().email(AccountError.INVALID_EMAIL.code);
...
export const displayName = z.string()
    ...
    .regex(/^[\p{L}\p{N} ]+$/u, AccountError.INVALID_DISPLAY_NAME.code);

export const username = z.string()
    .min(3, AccountError.USERNAME_TOO_SHORT.code)
    .max(20, AccountError.USERNAME_TOO_LONG.code)
    .regex(/^\w+$/, AccountError.INVALID_USERNAME.code);
